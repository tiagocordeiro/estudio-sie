...
class DashboardViewTest(TestCase):
    def setUp(self):
        ...
        # User account
        self.user_admin = User.objects.create_user(username='jacob',
                                                   email='jacob@…',
                                                   password='top_secret')

        self.company = Company.objects.create(name='My Business',
                                              address='FooBar street, no#',
                                              phone='(11) 1234-5678',
                                              website='https://foobar.com',
                                              facebook='https://foobar.com',
                                              instagram='https://foobar.com')
    ...
    def test_custom_user_profile_admin(self):
        ...
        get_inline_instances = CustomUserAdmin.get_inline_instances(
            self.user_admin, request)
...
class ProfileUpdateViewTest(TestCase):
    def setUp(self):
        ...
        # User support
        self.user_support = User.objects.create_user(username='john',
                                                     email='john@…',
                                                     password='top_secret')
        ...
        self.user_profile_support = UserProfile.objects.create(
            user=self.user_support, avatar=str(self.image))
